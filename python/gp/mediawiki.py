...
class MediaWikiGlue (MySQLGlue) :
	...
	def __init__( self, transport ) :
		super(MediaWikiGlue, self).__init__(self, transport)
...
class PageSet :
	...
	def create_table ( self, ) :
		...
		if ( not table  or  table == '?' ) :
			table = "gp_temp_" + self.glue.next_id()
			t = " TEMPORARY "
	...
	def make_source ( self, ns = None, ids_only = False ) :
		...
		if ( ns is not None ) :
			...
			if ( is_array(ns) ): select += " where page_namespace in " + self.glue.as_list( ns )
	...
	def strip_namespace ( self, ns, inverse = False ) :
		...
		if ( is_array(ns) ): sql +=  ( " not in " if inverse else " in " ) . self.glue.as_list( ns )
		else: sql += ( " != " if inverse else " = " ) . int(ns)
	...
	def expand_categories ( self, ns = None ) :
		...
		if (ns is not None) :
			if ( is_array(ns) ): sql += " where P.page_namespace in " + self.glue.as_list( ns )
	...
	def add_pages_in ( self, cat, ns, depth ) :
		if ( not self.add_subcategories(cat, depth) ): return False
